	...
	public:
		...
		class Creator {
			...
			Creator(Frames ... frames) {
				...
				static_assert(sizeof...(Frames) > 0, "SpriteAnimation must receieve at least one texture for its sequence");
		...
		public:
			...
			*/
			...
			Creator& setFrameDurations(Durations ... durations) {
				...
				if( numTimes != animation->frames.size() )
					throw new InvalidArgumentException("Number of animation durations does not match number of frames in SpriteAnimation");
			...
			*/
			Creator& setDuration(float duration) {
				if( !(duration > 0) )
					throw new InvalidArgumentException("Animation duration of SpriteAnimation must be larger than 0");
			...
			*/
			Creator& setFrameDuration(unsigned int frameIndex, float duration) {
				...
				if( !(frameIndex < animation->durations.size()) )
					throw new InvalidArgumentException("Frame index is out of bounds for SpriteAnimation");
			...
			/**
			 * @brief	Sets the collection of Assets that this SpriteAnimation asset belongs to
			*/
			...
			*/
			SpriteAnimation* finish() {
				if( finished )
					throw new InvalidStateException("SpriteAnimation::Creator has already finished");
		...
		/**
		 * @brief	Initialize a Creator object for a SpriteAnimation by a variable number of Texture pointers
